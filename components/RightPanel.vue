<template>
  <div class="space-y-4">
    ...
    <div class="card p-4">
      ...
      <div class="grid grid-cols-2 gap-2">
        <div class="text-center py-2.5 rounded-lg bg-gray-50 hover:bg-red-50 transition cursor-default">
          <div class="text-lg font-bold" style="color:#dd3333">{{ stats.articles || 0 }}</div>
        ...
        <div class="text-center py-2.5 rounded-lg bg-gray-50 hover:bg-orange-50 transition cursor-default">
          <div class="text-lg font-bold text-orange-500">{{ stats.comments || 0 }}</div>
    ...
    <div class="card p-4">
      <div class="flex items-center gap-2 mb-3">
        <span class="text-base">🔥</span>
        <h3 class="font-semibold text-gray-800 text-sm">热门文章</h3>
      </div>
      <ul v-if="hotArticles.length" class="space-y-1">
        <li v-for="(a, i) in hotArticles" :key="a.id">
          <NuxtLink :to="`/blog/${a.slug}`" class="flex items-start gap-2 py-1 group">
            ...
            <span class="text-sm text-gray-700 group-hover:text-[#dd3333] line-clamp-1">{{ a.title }}</span>
          </NuxtLink>
        </li>
      </ul>
      <p v-else class="text-xs text-gray-400 text-center py-2">暂无热门文章</p>
    </div>
    ...
    <div class="card p-4">
      ...
      <ul v-if="topUsers.length" class="space-y-2">
        <li v-for="u in topUsers" :key="u.id" class="flex items-center gap-2 text-sm">
          <div class="w-7 h-7 rounded-lg bg-gradient-to-br from-sakura-400 to-purple-500 text-white flex items-center justify-center text-xs font-bold flex-shrink-0">
            {{ (u.nickname || u.username)?.[0] }}
          </div>
          <div class="flex-1 min-w-0">
            <div class="text-gray-700 truncate">{{ u.nickname || u.username }}</div>
          </div>
          <span class="text-xs text-[#dd3333] font-medium">{{ u.article_count }}篇</span>
        </li>
      </ul>
